...
def read_raw_text(fname, input_keywords):
    start = time.time()
    all_text = []
    num_lines = sum(1 for line in open(fname, 'r', encoding='utf-8'))
...
def read_input_and_ground_truth(target_category_name):  
    ...
    with open(fname_euphemism_answer, 'r', encoding='utf-8') as fin:
        for line in fin:
    ...
    target_name = {}
    count = 0
    with open(fname_target_keywords_name, 'r', encoding='utf-8') as fin:
        for line in fin:
